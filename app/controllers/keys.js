...
const { getAllKeysByUserId, createKey, deleteKey, updateKey } = require('../database/methods/keys')
class Keys {
  ...
  update (request, h) {
    ...
    return (async () => {
      ...
      try {
        const key = await updateKey(userId, exchangeId, apiKeyEncoded, apiSecretEncoded)
        return {
          message: 'API key and secret updated!',
          key: key
        }
      } catch (err) {
        console.log('Unknown error while storing the API key and secret.', err)
